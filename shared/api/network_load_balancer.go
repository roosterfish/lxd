...
type NetworkLoadBalancerPort struct {
	...
	// Either a list of backends or a pool is required.
	// ---
	//  type: backend list
	//  required: no
	//  shortdesc: Backend name or names to forward to

	// TargetBackend contains backend names to load balance ListenPorts to
	...
	// TargetPool contains a pool to load balance ListenPorts to
	// Example: http-pool
...
// NetworkLoadBalancerPoolPost represents the fields of a new LXD network load balancer pool.
//
// swagger:model
//
// API extension: network_load_balancer_pool.
type NetworkLoadBalancerPoolPost struct {
