test_clustering_move() {
  ...
  # c1 cannot be moved to the default group since it is already a member
  ! LXD_DIR="${LXD_ONE_DIR}" lxc move c1 --target=@default || false
  ...
  # - node3 in group foobar3,default accepting direct targeting only
  ...
  # c2 can be moved to node1 by direct targeting.
