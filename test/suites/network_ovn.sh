test_network_ovn() {
  ...
  for ip in "${load_balancer_ips[@]}"; do
    ...
    [ "$(CLIENT_DEBUG="" SHELL_TRACING="" lxc network load-balancer pool delete "${ovn_network}" http 2>&1)" = 'Error: Pool "http" is still referenced by at least one load balancer port' ]
  ...
  [ "$(lxc network load-balancer pool info "${ovn_network}" http | yq '."load-balancers"."192.0.2.100:80" | map(select(.status == "unknown")) | length')" = "1" ]
  [ "$(lxc network load-balancer pool info "${ovn_network}" http | yq '."load-balancers"."[2001:db8:1:2::100]:80" | map(select(.status == "unknown")) | length')" = "1" ]
