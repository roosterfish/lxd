...
func (n *ovn) checkPoolHealthCheck(pool *api.NetworkLoadBalancerPool) (*loadBalancerHealthCheck, error) {
	...
	if ok {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed converting healtcheck interval %q: %w", intervalStr, err)
		}
	}
	...
	if ok {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed converting healtcheck timeout %q: %w", timeoutStr, err)
		}
	}
	...
	if ok {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed converting healtcheck success count %q: %w", successCountStr, err)
		}
	}
	...
	if ok {
		...
		if err != nil {
			return nil, fmt.Errorf("Failed converting healtcheck failure count %q: %w", failureCountStr, err)
		}
	}
...
func (n *ovn) poolHealthCheckSupported() error {
	...
	minVersion := [3]int{25, 9, 90}

	nbParts := strings.SplitN(nbVersion, ".", 3)
	if len(nbParts) != 3 {
		return fmt.Errorf("Failed parsing OVN version %q", nbVersion)
	}

	for i, min := range minVersion {
		v, err := strconv.Atoi(nbParts[i])
		if err != nil {
			return fmt.Errorf("Failed parsing OVN version segment %q: %w", nbParts[i], err)
		}
		...
		// Once any of the segments is greater than the minimum, we can stop checking.
		if v > min {
			break
		}
	}
...
func (n *ovn) LoadBalancerUpdate(listenAddress string, req api.NetworkLoadBalancerPut, clientType request.ClientType, force bool) error {
	...
	if clientType == request.ClientTypeNormal {
		...
		revert.Add(func() {
			...
			if err == nil {
				vips, _ := n.loadBalancerFlattenVIPs(net.ParseIP(curLoadBalancer.ListenAddress), portMaps)
...
func (n *ovn) loadBalancerPoolValidate(poolName string, pool api.NetworkLoadBalancerPoolPut) (*dbCluster.NetworksLoadBalancersPool, error) {
	var loadBalancerPoolDB *dbCluster.NetworksLoadBalancersPool
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		loadBalancerPoolDB, err = dbCluster.GetNetworksLoadBalancersPool(ctx, tx.Tx(), poolName)
		if err != nil {
			return err
		}
	...
	rules := map[string]func(value string) error{
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=protocol)
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=target_port)
		//
		// ---
		//  type: string
		//  required: yes
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=healthcheck)
		//
		// ---
		//  type: bool
		//  defaultdesc: `true`
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=healthcheck.interval)
		//
		// ---
		//  type: bool
		//  defaultdesc: `5`
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=healthcheck.timeout)
		//
		// ---
		//  type: bool
		//  defaultdesc: `3`
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=healthcheck.success_count)
		//
		// ---
		//  type: bool
		//  defaultdesc: `1`
		...
		// lxdmeta:generate(entities=network-load-balancer-pool; group=load-balancer-pool-properties; key=healthcheck.failure_count)
		//
		// ---
		//  type: bool
		//  defaultdesc: `1`
...
func (n *ovn) LoadBalancerPoolCreate(loadBalancerPool api.NetworkLoadBalancerPoolPost) error {
	revert := revert.New()
	defer revert.Fail()
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		poolID, err := query.Create(ctx, tx.Tx(), dbCluster.NetworksLoadBalancersPoolRow{
			NetworkID:   n.ID(),
		...
		return dbCluster.CreateNetworksLoadBalancersPoolConfig(ctx, tx.Tx(), poolID, loadBalancerPool.Config)
	})
	if err != nil {
		return err
	}
	...
	revert.Add(func() {
		_ = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
			return dbCluster.DeleteNetworksLoadBalancersPool(ctx, tx.Tx(), n.ID(), loadBalancerPool.Name)
		})
	})
...
func (n *ovn) loadBalancerPoolAddInstance(poolID int64, instance api.NetworkLoadBalancerPoolInstance) error {
	err := n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		targetPortNullable := sql.NullString{}
		if instance.TargetPort != "" {
			targetPortNullable.String = instance.TargetPort
			targetPortNullable.Valid = true
		}

		// Create load balancer pool instance DB record.
		_, err = query.Create(ctx, tx.Tx(), dbCluster.NetworksLoadBalancersPoolInstanceRow{
			PoolID:     poolID,
			InstanceID: int64(instanceID),
			TargetPort: targetPortNullable,
		})
		return err
	})
	if err != nil {
...
func (n *ovn) loadBalancerPoolUpdateInstance(poolID int64, instance api.NetworkLoadBalancerPoolInstance) error {
	err := n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		instanceDB := &dbCluster.NetworksLoadBalancersPoolInstanceRow{
			PoolID:     poolID,
			InstanceID: int64(instanceID),
			TargetPort: targetPortNullable,
		}
		...
		return dbCluster.UpdateNetworkLoadBalancersPoolInstanceRow(ctx, tx.Tx(), instanceDB)
	})
	if err != nil {
		return err
	}
...
func (n *ovn) loadBalancerPoolRemoveInstance(poolID int64, instanceName string) error {
	err := n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		return dbCluster.DeleteNetworksLoadBalancersPoolInstanceRow(ctx, tx.Tx(), poolID, int64(instanceID))
	})
	if err != nil {
		return err
	}
...
func (n *ovn) LoadBalancerPoolUpdate(poolName string, loadBalancerPoolPut api.NetworkLoadBalancerPoolPut) error {
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		loadBalancerPool, err = loadBalancerPoolDB.ToAPI(ctx, tx.Tx())
		return err
	})
	if err != nil {
		return err
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		poolDBNew := &dbCluster.NetworksLoadBalancersPoolRow{
		...
		return dbCluster.UpdateNetworksLoadBalancersPool(ctx, tx.Tx(), poolDBNew, loadBalancerPoolPut.Config)
	})
	if err != nil {
		return err
	}
	...
	revert.Add(func() {
		_ = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
			return dbCluster.UpdateNetworksLoadBalancersPool(ctx, tx.Tx(), &loadBalancerPoolDB.Row, loadBalancerPool.Config)
		})
	})
...
func (n *ovn) LoadBalancerPoolDelete(poolName string) error {
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		return dbCluster.DeleteNetworksLoadBalancersPool(ctx, tx.Tx(), n.ID(), poolName)
	})
	if err != nil {
		return err
	}
...
func (n *ovn) LoadBalancerPoolState(poolName string) (*api.NetworkLoadBalancerPoolState, error) {
	...
	err := n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		poolDB, err := dbCluster.GetNetworksLoadBalancersPool(ctx, tx.Tx(), poolName)
		if err != nil {
			return err
		}

		pool, err = poolDB.ToAPI(ctx, tx.Tx())
		if err != nil {
			return err
		}
