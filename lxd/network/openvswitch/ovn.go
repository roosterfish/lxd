...
func (o *OVN) LoadBalancerApply(loadBalancerName OVNLoadBalancer, routers []OVNRouter, switches []OVNSwitch, vips ...OVNLoadBalancerVIP) error {
	...
	for _, vip := range vips {
		...
		for _, target := range vip.Targets {
			...
			ipStr := ipToString(target.Address)
...
func (o *OVN) GetNorthdVersion() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Failed unquoting northd_internal_version %q: %w", output, err)
	}
	...
	if len(rowParts) < 1 {
		return "", fmt.Errorf("Failed splitting northd_internal_version %q", output)
	}

	return rowParts[0], nil
}
