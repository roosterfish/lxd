...
func (d *hpe) commonVolumeRules() map[string]func(value string) error {
	return map[string]func(value string) error{
		// lxdmeta:generate(entities=storage-hpe; group=volume-conf; key=block.filesystem)
		...
		// lxdmeta:generate(entities=storage-hpe; group=volume-conf; key=block.mount_options)
		//
		// ---
		//  type: string
		//  condition: block-based volume with content type `filesystem`
		...
		// lxdmeta:generate(entities=storage-hpe; group=volume-conf; key=size)
...
func (d *hpe) CreateVolumeFromBackup(vol VolumeCopy, srcBackup backup.Info, srcData io.ReadSeeker, op *operations.Operation) (VolumePostHook, revert.Hook, error) {
...
func (d *hpe) CreateVolumeFromCopy(vol VolumeCopy, srcVol VolumeCopy, allowInconsistent bool, op *operations.Operation) error {
	...
	// HPE Storage does not allow copying snapshots along with the volume. Therefore, we
...
func (d *hpe) CreateVolumeFromMigration(vol VolumeCopy, conn io.ReadWriteCloser, volTargetArgs migration.VolumeTargetArgs, preFiller *VolumeFiller, op *operations.Operation) error {
...
func (d *hpe) RefreshVolume(vol VolumeCopy, srcVol VolumeCopy, refreshSnapshots []string, allowInconsistent bool, op *operations.Operation) error {
	revert := revert.New()
	defer revert.Fail()
...
func (d *hpe) refreshVolume(vol VolumeCopy, srcVol VolumeCopy, refreshSnapshots []string, allowInconsistent bool, op *operations.Operation) (revert.Hook, error) {
	...
	if !srcVol.IsSnapshot() && len(refreshSnapshots) > 0 {
		...
		// HPE Storage does not allow copying snapshots along with the volume. Therefore,
...
func (d *hpe) HasVolume(vol Volume) (bool, error) {
	volName, err := d.getVolumeName(vol)
	if err != nil {
		return false, err
	}
...
func (d *hpe) FillVolumeConfig(vol Volume) error {
...
func (d *hpe) GetVolumeUsage(vol Volume) (int64, error) {
	volName, err := d.getVolumeName(vol)
	if err != nil {
		return -1, err
	}
...
func (d *hpe) MountVolume(vol Volume, op *operations.Operation) error {
	...
	// Activate HPE Storage volume if needed.
	...
	case ContentTypeFS:
		...
		if !filesystem.IsMountPoint(mountPath) {
			...
			d.logger.Debug("Mounted HPE Storage volume", logger.Ctx{"volName": vol.name, "dev": volDevPath, "path": mountPath, "options": mountOptions})
		}
...
func (d *hpe) RenameVolume(vol Volume, newVolName string, op *operations.Operation) error {
	// Renaming a volume won't change an actual name of the HPE Storage volume.
	return nil
}

// RestoreVolume restores a volume from a snapshot.
func (d *hpe) RestoreVolume(vol Volume, snapVol Volume, op *operations.Operation) error {
...
func (d *hpe) MigrateVolume(vol VolumeCopy, conn io.ReadWriteCloser, volSrcArgs *migration.VolumeSourceArgs, op *operations.Operation) error {
...
func (d *hpe) BackupVolume(vol VolumeCopy, tarWriter *instancewriter.InstanceTarWriter, optimized bool, snapshots []string, op *operations.Operation) error {
	return genericVFSBackupVolume(d, vol, tarWriter, snapshots, op)
}

// CreateVolumeSnapshot creates a snapshot of a volume.
func (d *hpe) CreateVolumeSnapshot(snapVol Volume, op *operations.Operation) error {
...
// is not copied.
func (d *hpe) createVolumeSnapshot(snapVol Volume, snapshotVMfilesystem bool, op *operations.Operation) error {
	revert := revert.New()
	defer revert.Fail()
...
func (d *hpe) DeleteVolumeSnapshot(snapVol Volume, op *operations.Operation) error {
	parentVol := snapVol.GetParent()
	parentVolName, err := d.getVolumeName(parentVol)
	if err != nil {
		return err
...
func (d *hpe) MountVolumeSnapshot(snapVol Volume, op *operations.Operation) error {
	revert := revert.New()
	defer revert.Fail()
	...
	// A HPE Storage snapshot cannot be mounted. To mount a snapshot, a new volume
...
func (d *hpe) UnmountVolumeSnapshot(snapVol Volume, op *operations.Operation) (bool, error) {
...
// VolumeSnapshots returns a list of HPE Storage snapshot names for the given volume (in no particular order).
func (d *hpe) VolumeSnapshots(vol Volume, op *operations.Operation) ([]string, error) {
	volName, err := d.getVolumeName(vol)
	if err != nil {
		return nil, err
	}
...
// match those provided. Note that additional snapshots may exist within the HPE Storage pool
// if protection groups are configured outside of LXD.
func (d *hpe) CheckVolumeSnapshots(vol Volume, snapVols []Volume, op *operations.Operation) error {
...
func (d *hpe) RenameVolumeSnapshot(snapVol Volume, newSnapshotName string, op *operations.Operation) error {
	// Renaming a volume snapshot won't change an actual name of the HPE Storage volume snapshot.
	return nil
}
