...
func doProfileUpdate(ctx context.Context, s *state.State, p api.Project, profileName string, profile *api.Profile, req api.ProfilePut) error {
	...
	for _, inst := range insts {
		...
		err = s.DB.Cluster.Transaction(ctx, func(ctx context.Context, tx *db.ClusterTx) error {
			...
			poolInstRows, err := cluster.GetNetworksLoadBalancersPoolInstanceRowsByID(ctx, tx.Tx(), int64(inst.ID))
			if err != nil {
				return err
			}
			...
			for _, poolInstRow := range poolInstRows {
				...
				pool, err := cluster.GetNetworksLoadBalancersPoolRowByID(ctx, tx.Tx(), poolInstRow.PoolID)
				if err != nil {
					return err
				}
